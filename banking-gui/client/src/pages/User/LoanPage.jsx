  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/loans/${user.CUSTOMER_ID}`)
      ...
      .then(data => {
        ...
        const activeLoans = allLoans.filter(loan => loan.LOAN_STATE === 'ACTIVE');
        setLoans(activeLoans);
        if (activeLoans.length > 0) setSelectedLoanId(activeLoans[0].LOAN_ID);
        setLoading(false);
      })
      .catch((err) => {
...
useEffect(() => {
  ...
  const activeLoan = loans.find(loan => loan.LOAN_ID === selectedLoanId) || null;
  ...
  // Uses loanTerm from state — updates live as user picks 12 / 24 / 36
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl relative">
            ...
            <div className="space-y-6">
              ...
              {/* Estimated Monthly — updates live */}
              <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs font-bold text-gray-400 bg-white border border-gray-200 px-2 py-0.5 rounded-full">
                    5% interest · {loanTerm} months
                  </span>
                </div>
                <p className="text-4xl font-black text-gray-900">${estimatedMonthly}</p>
                <p className="text-xs text-gray-400 mt-2">
                  Total repayment: <span className="font-bold text-gray-600">${(loanAmount * 1.05).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </p>
              </div>

              <button
                onClick={handleApply}
                className="w-full py-4 bg-gray-900 text-white font-black rounded-xl hover:bg-black transition-colors"
              >
                Submit Application
              </button>
            </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className={`flex-1 py-3 text-white font-bold rounded-xl shadow-lg transition-all ${
                    !userAccounts || userAccounts.length === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-[#004a99] hover:bg-[#003d7a]'
                  }`}
                >
                  Confirm Payment
                </button>
